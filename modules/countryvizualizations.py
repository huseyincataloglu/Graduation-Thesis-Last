...
def plot_countries_prod_map(df,countries,years,locations,methods = None):
    ...
    if len(locations) > 0 :
        ...
        figure = px.choropleth(grouped_filteredcountryproductions,locations="Country",locationmode="country names",color="Production")
        ...
        figure = px.choropleth(filteredcountryproductions,locations="Country",locationmode="country names",color="Production")
        figure.update_layout(
            font=dict(size=16),  # Yazı boyutu
            coloraxis_colorbar=dict(
                title="Production",        # Renk skalası başlığı
...
def plot_countryprod_by_time(df,countries,years,locations,methods = None):
    ...
    if len(locations) > 0 :
        ...
        figure = px.area(grouped_filteredcountryproductions,x = "Years",y = "Production",color="Country",color_discrete_sequence=px.colors.sequential.RdBu)
        figure.update_layout(
            font=dict(size=16),  # Yazı boyutu
            xaxis=dict(
                gridwidth=1,
    ...
    figure = px.area(df_melted,x = "Years",y = "Production",color="Country",color_discrete_sequence=px.colors.sequential.RdBu)
    figure.update_layout(
        font=dict(size=16),  # Yazı boyutu
        xaxis=dict(
            gridwidth=1,
